...
const fileAttachmentAlreadyAttachedNotification = ({
  fileName,
}: {
  fileName: string;
}): Notification => ({
...
const fileAttachmentInvalidTypeNotification = ({
  fileName,
}: {
  fileName: string;
}): Notification => ({
...
const fileDownloadInvalidTypeNotification = ({
  fileName,
}: {
  fileName: string;
}): Notification => ({
  ...
  message: `${shortenFileName(
    fileName
  )} can’t be downloaded because the file type isn’t supported.`,
  type: "error",
  timeout: 10000,
});
