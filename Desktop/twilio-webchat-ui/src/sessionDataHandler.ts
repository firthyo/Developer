...
async function contactBackend<T>(
...
export const sessionDataHandler = {
  ...
  tryResumeExistingSession(): Token | null {
    ...
    if (Date.now() >= new Date(storedTokenData.expiration).getTime()) {
      log.debug(
        "sessionDataHandler: token expired, ignoring existing sessions"
      );
      return null;
    }

    log.debug(
      "sessionDataHandler: existing token still valid, using existing session data"
    );
  ...
  async getUpdatedToken(): Promise<Token> {
    ...
    try {
      ...
      throw Error(
        `Something went wrong when trying to get an updated token: ${e}`
      );
    }
